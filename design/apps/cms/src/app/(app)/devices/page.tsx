...
/**
 * EasySignage — Dispositivos (Enterprise)
 * Substitui apps/cms/src/app/(app)/devices/page.tsx
 *
 * Correções:
 * - Estado como BADGE (não texto cru) + plataforma com rótulo humano
 * - Conexão com dot + label (não depende só de cor — §19)
 * - Filtros em pílula (Todos / Online / Offline) + selects
 * - Ações como botões de ícone (lucide)
 * - Confirmação de exclusão: TODO trocar window.confirm por <Modal> (§13.6)
 */
...
function PlatformIcon({ platform }: { platform: string }) {
  const Icon = platform === 'web' ? Globe : platform === 'tv_browser' || platform === 'android_browser' ? Tv : MonitorPlay;
  return (
    <span
      style={{
...
export default function DevicesPage() {
  ...
  async function onDelete(d: DeviceRow) {
    // TODO: substituir por modal de confirmação (§13.6) em vez de window.confirm
    const ok = window.confirm(`Eliminar o dispositivo «${d.name}»? Esta ação não pode ser desfeita.`);
    if (!ok) return;
    setError(null);
    setDeletingId(d.id);
    try {
    ...
    <>
      ...
      <div className="surface-filters">
        <div className="filter-pills" role="group" aria-label="Conexão">
          <button className={online === '' ? 'is-active' : ''} onClick={() => setOnline('')}>Todos {total}</button>
          <button className={online === 'true' ? 'is-active' : ''} onClick={() => setOnline('true')}>Online {onlineCount}</button>
          <button className={online === 'false' ? 'is-active' : ''} onClick={() => setOnline('false')}>Offline {total - onlineCount}</button>
        </div>
        <div style={{ flex: 1 }} />
        <select className="select" value={siteId} onChange={(e) => setSiteId(e.target.value)} aria-label="Site">
          <option value="">Todos os sites</option>
          {sites.map((s) => (<option key={s.id} value={s.id}>{s.name}</option>))}
        </select>
        <select className="select" value={platform} onChange={(e) => setPlatform(e.target.value)} aria-label="Plataforma">
      ...
      {items && items.length === 0 && (
        <EmptyState
          ...
          action={<Link href="/devices/new" className="btn btn--primary">Novo dispositivo</Link>}
        />
      )}
              ...
              {items.map((d) => {
                ...
                return (
                  <tr key={d.id}>
                    ...
                    <td><StatusPill label={st.label} tone={st.tone} /></td>
                    <td><ConnectionPill state={isOnline(d.lastSeenAt) ? 'on' : 'off'} /></td>
                    <td className="cell-sub">{formatDateTimePtBr(d.lastSeenAt)}</td>
                    <td>
                      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 6 }}>
                        <Link href={`/devices/${d.id}`} className="btn btn--icon" aria-label="Editar" title="Editar">
                          <Pencil aria-hidden />
                        </Link>
                        <button
                          type="button"
      ...
      {items && items.length > 0 && (
        <div className="table-footer">
          <span>Mostrando {items.length} de {items.length} dispositivos</span>
        </div>
      )}
    </>
  );
