/**
 * Lógica pura de despacho de comandos remotos (PR 7.2) — mesma forma de
 * `apps/androidtv-player` (`CommandDispatcher.kt`) e do Electron
 * (`easysignage-bridge.ts`): recebe `{ method, args }`, valida o essencial e delega para
 * uma implementação de plataforma injetada, sem depender de `window`/`document`/`webOS`
